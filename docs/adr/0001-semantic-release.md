...
- `gh skill publish` runs in the same workflow after a release is published, using the release's `new_release_published` output.
...
- `gh skill publish` runs only when a release was actually published; CI runner `gh` must be recent enough to expose the `skill` preview command.
